const db = require('../model/db_function');
...
module.exports.editTask = function (req, res) {
    db.update(req.body, req.params.id).then(results => results? res.json(results) : res.status(400).json({err: 'Cat not found'}))
        .catch(err => res.status(400).json({err: err.message}))
};

module.exports.deleteTask = function (req, res) {
    db.delete(req.params.id).then(results => results ? res.json(results) : res.status(400).json({err: 'Cat not found'}))
        .catch(err => res.status(400).json({err: err.message}))
};